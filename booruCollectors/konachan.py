...
def collector(downTag):
    print("Starting Konachan Collector.")
    if len(downTag) > 1:
        joined_tags = "+".join(downTag)
    elif len(downTag) == 1:
        joined_tags = downTag[0]
    baseURL = "https://konachan.com/post.json?limit=100"
    page = 1
    site = "konachan"
    downloadList = []
    end = 0
    ...
    dir_tag = tag
    tag = tag + "/"
    tagPath = '/app/downloads/konachan/' + tag
    if not os.path.exists(tagPath):
        os.makedirs(tagPath)
    while True:
        postCount = 0
        url = f"{baseURL}&page={page}&tags={joined_tags}"
        ...
        try:
            ...
            if end != 1:
                print(f"Found {postCount} posts on page {page} of Konachan.")
                page = page + 1
