...
def collector(downTag):
    print("Starting Yandere Collector.")
    if len(downTag) > 1:
        joined_tags = "+".join(downTag)
    elif len(downTag) == 1:
        joined_tags = downTag[0]
    baseURL = "https://yande.re/post.json?limit=1000"
    page = 1
    site = "yandere"
    downloadList = []
    end = 0
    ...
    dir_tag = tag
    tag = tag + "/"
    tagPath = '/app/downloads/yandere/' + tag
    ...
    while True:
        ...
        try:
            ...
            if end != 1:
                print(f"Found {postCount} posts on page {page} of Yandere.")
                page = page + 1
