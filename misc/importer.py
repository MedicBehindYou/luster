...
def bulk(filename):
    try:
        ...
        misc.logger.log(f'Entries from "{filename}" imported successfully.')
    except Exception as e:
        misc.logger.log(f'Error bulk importing entries from "{filename}": {e}')

def single(name, siteNum: int = 0, genre_ids: str = 0):
    try:
        conn = sqlite3.connect(DATABASE_DB, timeout=20)
        cursor = conn.cursor()
        ...
        misc.logger.log(f'Single entry "{sanitized_name}" with site "{siteNum}" imported successfully.')
    except Exception as e:
        misc.logger.log(f'Error importing single entry "{name}": {e}')
    return
